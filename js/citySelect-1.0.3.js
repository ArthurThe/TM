...
(function ($, window) {
    ...
    functionality.template = [
        '<div class="city-pavilion hide">',
            '<div class="city-tabs">',
                ...
                '<a href="javascript:;" class="tab-a" data-letter="ABCDE">ABCDE</a>',
                '<a href="javascript:;" class="tab-a" data-letter="FGHJ">FGHJ</a>',
                '<a href="javascript:;" class="tab-a" data-letter="KLMNP">KLMNP</a>',
                '<a href="javascript:;" class="tab-a" data-letter="QRSTW">QRSTW</a>',
                '<a href="javascript:;" class="tab-a" data-letter="XYZ">XYZ</a>',
            '</div>',
            '<div class="city-cont">',
                '{cont}',
            '</div>',
    ...
    functionality.singleAchieve = function (event) {
        ...
        this.$selector.find('.city-input').before('<span data-id="'+ id +'" data-parentid="'+ parentId +'">'+ name +'</span>').find('.input-search').val('');
    ...
    Cityselect.prototype.createSubject = function () {
        ...
        if (!configure.search) {
            self.$selector.find('.city-input').addClass('not-search').html('<em>' +'</em>');
    ...
    Cityselect.prototype.bindEvent = function (event) {
        ...
        //显示城市-弹窗
        $selector.on('mouseover.cityselect', '.city-info', $.proxy(functionality.showDrop, self));
        ...
        //点击清空
        $selector.on('mouseout.cityselect', '.empty', $.proxy(self.clear, self));
    ...
    Cityselect.prototype.unBindEvent = function (event) {
        ...
        $selector.off('mouseout.cityselect', '.empty');
            ...
            for (var i = 0; i < cityArray.length; i++) {
                ...
                $.each(self.newCityData, function (key, value) {
                    ...
                    if (cityArray[i] === value.name) {
                        ...
                        //选中的值拼接dom
                        joinSpan = '<span data-id="'+ value.id +'" data-parentid="'+ value.parentId +'">'+ name +'</span>';
    ...
    //点击以外的地方就隐藏
    $(document).on('mouseout.cityselect', function (event) {
